...
export class CryptoService {
   ...
   */
  set(value: string, keys: string = null): string {
  ...
  get(value: string, keys: string = null): string {
    let key: CryptoJS.lib.WordArray;
    let iv: CryptoJS.lib.WordArray;
    if (keys) {
      key = CryptoJS.enc.Utf16.parse(keys);
  ...
  decryptPayload(responseBody: string, password: string = this.keyId) {
   ...
   * @memberof CryptoService
   */
  generateRandomString(length: number = 16, caseType: number = 5): string {
    let pass: any = '';
